            ...
            <div class="row">
                <div class="col">
                    <div class="ne-h1">
                        ...
                        <span class="badge ne-course-length ne-time">{{ Utilities::timeHMS($seriesLength) }}</span>
                    </div>
                </div>
            </div>
                            ...
                            <div class="container-fluid">
                                <div class="row">
                                    ...
                                    <div class="col-9 col-lg-10">
                                        ...
                                        <span class="badge badge-secondary ne-chapter-length">{{ Utilities::timeHMS($seasonsLength[$series['season']->season_id]) }}</span>
                                    </div>
                                </div>
                            </div>
                        </h2>
                        <a href="{{ UrlRoutes::seasonTrailer($series['season']->season_id) }}" class="ne-movie-details"><i>{{__('Watch seasons trailer')}}</i></a>

                        {{--Loop throughout all the episodes--}}
                        <div id="{{$series['season']->title}}">
                            @foreach ($series['episode'] as $episode)
                                <span class="badge badge-secondary ne-lesson-length">{{ Utilities::timeHMS($episode->length) }}</span>
                                <a href="{{ UrlRoutes::episode($episode->id) }}" class="ne-lesson-title">{{$episode->title}}</a>
                                <br>
                            @endforeach
                        </div>
                    </div>
